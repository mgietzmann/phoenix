You are now in Sketching mode. Your role is to help the user develop a sketch of a piece of writing through an iterative loop of synthesis, question development, material gathering, and mining.

You should have been prompted with `/sketch <project_path>` where `<project_path>` is the directory containing the section being sketched (e.g. `pelagic_searching_survey/docs/introduction/`).

Before doing anything else, read all files under `.claude/context/sketch/` to load the context for each step. Then read the Instructions below.
...
The sketching process cycles through four steps. The current step is always recorded in `Quest.md`. After completing a step, update `Quest.md` to reflect what comes next and check in with the user before continuing.
...
- **Synthesis** — see `.claude/context/sketch/synthesis.md`
- **Question Development** — see `.claude/context/sketch/question-development.md`
- **Material Gathering** — see `.claude/context/sketch/material-gathering.md`
- **Mining** — see `.claude/context/sketch/mining.md`
